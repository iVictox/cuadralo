...
import {
    ...
    Dumbbell, Film, ChevronRight, AlertCircle
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import Image from "next/image";
import { api } from "@/utils/api"; // Importamos nuestra utilidad de conexión

export default function RegisterPage() {
    const router = useRouter();
    const [step, setStep] = useState(0); // 0 = Registro, 1-5 = Onboarding
    const [error, setError] = useState("");
    const [isLoading, setIsLoading] = useState(false);

    // Estado gigante para guardar todo
    const [formData, setFormData] = useState({
        name: "", email: "", password: "", confirmPassword: "",
        birthDate: "", gender: "", photo: null,
        bio: "", interests: [], preferences: { ageRange: [18, 30], distance: 50, show: "everyone" }
    });
    ...
    const handleRegisterStart = (e) => {
        ...
        nextStep(); // Pasa del registro básico al onboarding
    };
    ...
    const handleFinalSubmit = async () => {
        ...
        try {
            ...
            // NOTA: 'photo' se envía vacía por ahora porque el backend espera un string
            // y aún no implementamos la subida de archivos a la nube (S3/Cloudinary).
            const payload = {
                ...formData,
                photo: "",
            };

            // Llamada a la API (POST /register)
            const response = await api.post("/register", payload);

            console.log("Registro exitoso:", response);

            // Redirigir al login tras un breve delay para ver el éxito
            setTimeout(() => {
                router.push("/login");
            }, 1000);
            ...
            setIsLoading(false); // Solo paramos la carga si hay error
        }
    };
    ...
    // Manejador de Foto (Local Preview)
    ...
    // Cálculo de progreso (Del paso 1 al 5)
    ...
    return (
        <div className="min-h-screen w-full bg-[#0f0518] flex items-center justify-center relative overflow-hidden py-10">
            ...
            >
                ...
                {/* BARRA DE PROGRESO (Visible del paso 1 en adelante) */}
                {step > 0 && (
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden min-h-[550px] flex flex-col">
                    ...
                    {/* Botón Atrás (Si no estamos en el paso 0 y no está cargando) */}
                    {step > 0 && !isLoading && (
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {step === 1 && (
                            ...
                            >
                                ...
                                <input
                                    type="date"
                                    className="w-full bg-black/30 border border-white/20 rounded-xl p-4 text-white text-center text-lg focus:border-cuadralo-pink outline-none mb-6"
                                ...
                                <div className="flex gap-4 w-full mb-8">
                                    {['Hombre', 'Mujer', 'Otro'].map((g) => (
                                        <button
                                            ...
                                            className={`flex-1 py-3 rounded-xl border transition-all ${formData.gender === g ? 'bg-cuadralo-pink border-cuadralo-pink text-white' : 'border-white/10 hover:bg-white/5 text-gray-400'}`}
                                        >
                                            {g}
                                        </button>
                                    ))}
                        ...
                        {step === 2 && (
                            ...
                            >
                                ...
                                <div className="relative w-48 h-48 mb-8 group cursor-pointer">
                                    ...
                                    <div className={`w-full h-full rounded-full border-4 flex items-center justify-center overflow-hidden transition-all ${formData.photo ? 'border-cuadralo-pink' : 'border-dashed border-white/20 hover:border-white/50 bg-black/20'}`}>
                                        {formData.photo ? (
                                    ...
                                    {/* Botón flotante */}
                                    <div className="absolute bottom-2 right-2 bg-cuadralo-pink p-2 rounded-full text-white shadow-lg z-10">
                                        {formData.photo ? <Check size={20} /> : <Camera size={20} />}
                                    </div>
                                </div>

                                <NextButton onClick={nextStep} disabled={!formData.photo} />
                                <button onClick={nextStep} className="mt-4 text-xs text-gray-500 hover:text-white">Saltar por ahora</button>
                            </motion.div>
                        )}
                        ...
                        {step === 3 && (
                            ...
                            >
                                ...
                                <textarea
                                    placeholder="Me gusta el café, los gatos y..."
                                    className="w-full h-32 bg-black/30 border border-white/20 rounded-xl p-4 text-white focus:border-cuadralo-pink outline-none resize-none mb-2"
                                    value={formData.bio}
                                    ...
                                    {interestsList.map((interest) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${active ? 'bg-cuadralo-pink/20 border-cuadralo-pink text-white' : 'bg-black/20 border-white/10 text-gray-400 hover:bg-white/5'}`}
                                            >
                        ...
                        {step === 5 && (
                            <motion.div
                                ...
                                className="flex flex-col h-full items-center text-center justify-center"
                            >
                                <div className="w-16 h-16 bg-gradient-to-tr from-cuadralo-pink to-purple-600 rounded-full flex items-center justify-center text-white mb-4 shadow-lg shadow-purple-500/30">
                                ...
                                <div className="w-full mb-6 text-left">
                                    <div className="flex justify-between mb-2">
                                        <label className="text-sm font-bold text-gray-400">Distancia Máxima</label>
                                        <span className="text-sm font-bold text-white">{formData.preferences.distance} km</span>
                                    </div>
                                    <input
                                        type="range" min="1" max="100"
                                        value={formData.preferences.distance}
                                        onChange={(e) => handleChange("preferences", { ...formData.preferences, distance: e.target.value })}
                                        className="w-full accent-cuadralo-pink h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer"
                                    />
                                </div>
                                ...
                                <div className="w-full mb-8 text-left">
                                    ...
                                    <div className="flex bg-black/30 p-1 rounded-xl">
                                        {['Hombres', 'Mujeres', 'Todos'].map((opt) => (
                                            <button
                                                key={opt}
                                                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-colors ${formData.preferences.show === opt ? 'bg-gray-700 text-white shadow-md' : 'text-gray-500 hover:text-gray-300'}`}
                                                onClick={() => handleChange("preferences", { ...formData.preferences, show: opt })}
                                            >
                                                {opt}
                                            </button>
                                        ))}
                                ...
                                {/* Mostrar Error del Backend si hay */}
                                {error && <ErrorMessage msg={error} />}

                                <button
                                    onClick={handleFinalSubmit}
                                    disabled={isLoading}
                                    className={`w-full bg-gradient-to-r from-cuadralo-pink to-cuadralo-purple py-4 rounded-xl font-bold text-white text-lg shadow-xl flex items-center justify-center gap-2 transition-all ${isLoading ? 'opacity-70 cursor-wait' : 'hover:scale-[1.02]'}`}
                                >
                                    {isLoading ? "Creando perfil..." : (
...
function InputGroup({ icon, type = "text", placeholder, value, onChange }) {
    return (
        <div className="relative group">
            <div className="absolute left-4 top-3.5 text-gray-500 group-focus-within:text-cuadralo-pink transition-colors">
                {icon}
            </div>
            <input
                type={type}
...
function ErrorMessage({ msg }) {
    return (
        <div className="w-full bg-red-500/10 border border-red-500/30 p-3 rounded-xl flex items-center gap-2 text-red-400 text-xs mb-4">
            <AlertCircle size={16} />
            {msg}
        </div>
    );
...
// Componente Botón Siguiente
function NextButton({ onClick, disabled, text = "Siguiente" }) {
    return (
        <button
            onClick={onClick}
            disabled={disabled}
            className="w-full bg-white text-black py-3 rounded-xl font-bold text-lg shadow-lg hover:scale-[1.02] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:hover:scale-100"
        >
