...
export default function RegisterPage() {
  ...
  const [isUploading, setIsUploading] = useState(false); // Estado para la subida de foto
  
  // Estado gigante para guardar todo
  const [formData, setFormData] = useState({
      name: "", email: "", password: "", confirmPassword: "",
      birthDate: "", gender: "", photo: "", // photo será la URL del servidor
      bio: "", interests: [], preferences: { ageRange: [18, 30], distance: 50, show: "everyone" }
  });

  // --- LISTA DE INTERESES (ACTUALIZADA Y COMPLETA) ---
  const interestsList = [
  ...
  // --- SUBIDA DE FOTO REAL AL SERVIDOR ---
  const handlePhotoUpload = async (e) => {
      ...
      try {
          ...
          console.log("Foto guardada en:", serverUrl);

          // 2. Guardar URL real en el estado
  ...
  const toggleInterest = (id) => {
      setFormData(prev => {
          ...
          if (exists) {
              ...
              // Limitar a 5 si quieres, o dejar libre
      ...
      >
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden min-h-[550px] flex flex-col">
            ...
            <AnimatePresence mode="wait">
                ...
                {/* --- PASO 2: FOTO DE PERFIL (CON SUBIDA REAL) --- */}
                {step === 2 && (
                    <motion.div 
                        key="step2"
                        initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }}
                    ...
                    >
                        ...
                        <div className="relative w-48 h-48 mb-8 group cursor-pointer">
                            ...
                            <div className={`w-full h-full rounded-full border-4 flex items-center justify-center overflow-hidden transition-all relative ${formData.photo ? 'border-cuadralo-pink' : 'border-dashed border-white/20 hover:border-white/50 bg-black/20'}`}>
                                {isUploading ? (
                                    <Loader2 size={48} className="text-cuadralo-pink animate-spin" />
                                ) : formData.photo ? (
                ...
                {/* --- PASO 4: INTERESES (LISTA COMPLETA) --- */}
                {step === 4 && (
                    <motion.div 
                        key="step4"
                        initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }}
