...
export default function AdminLayout({ children }) {
  ...
  // Expandir la categoría correcta según la ruta actual
  useEffect(() => {
    const currentCategory = menuCategories.find(cat => 
      cat.items.some(item => pathname === item.path || pathname.startsWith(item.path + '/'))
    );
    if (currentCategory) {
      setOpenCategory(currentCategory.title);
    }
              ...
              <div key={idx} className="mb-1.5">
                ...
                >
                  <div className="flex items-center gap-3">
                    <category.icon size={18} className={isOpen ? "text-purple-500" : "text-gray-500"} />
                    <span className="tracking-wide">{category.title}</span>
                  </div>
                  <ChevronDown size={14} className={`transition-transform duration-300 ${isOpen ? "rotate-180 text-purple-500" : "text-gray-600"}`} />
                </button>
