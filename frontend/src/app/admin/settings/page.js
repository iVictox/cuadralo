...
import { Settings, Save, Server, Euro, ShieldAlert, Sparkles, RefreshCw } from "lucide-react";
...
export default function AdminSettings() {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (data && Object.keys(data).length > 0) {
          // Soporte si existía el parámetro viejo de USD
          setSettings(prev => ({ 
              ...prev, 
              ...data,
              vip_price_eur: data.vip_price_eur || data.vip_price_usd || "4.99"
          }));
        }
      } catch (error) {
  ...
  // ✅ Obtiene la Tasa Oficial del EURO BCV
  const fetchExternalRate = async () => {
    ...
    try {
        ...
        if (data && data.promedio) {
            ...
            alert(`✅ Éxito: Tasa oficial EURO BCV obtenida: ${data.promedio} Bs`);
        } else {
  ...
  const calculatedBs = (parseFloat(settings.vip_price_eur || 0) * parseFloat(settings.bs_exchange_rate || 0)).toFixed(2);
        ...
        <div className="space-y-8">
            <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden shadow-2xl">
                <div className="bg-gray-950 px-6 py-4 border-b border-gray-800 flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-bold text-white">Monetización (EUROS)</h2>
                </div>
                <div className="p-6 space-y-6">
                    
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Precio Suscripción</label>
                            <div className="relative">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">€</span>
                                <input type="number" step="0.01" name="vip_price_eur" value={settings.vip_price_eur} onChange={handleChange} className="w-full bg-gray-950 border border-gray-800 rounded-xl pl-8 pr-4 py-3 text-green-400 font-mono font-bold outline-none focus:border-green-500" />
                            </div>
                        </div>
                        <div>
                    ...
                    <div className="bg-gray-950 p-5 rounded-xl border border-gray-800 relative">
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <Euro size={14} className="text-green-500"/> Tasa de Cambio Euro
                            </label>
                            <button 
                            type="button" 
