  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? <div className="col-span-3 text-center py-10 text-purple-400 animate-pulse font-bold">Cargando publicaciones...</div> : posts.length === 0 ? <div className="col-span-3 text-center py-10 text-gray-500">No hay posts.</div> : posts.map((post) => (
          <div key={post.id} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-lg flex flex-col">
            ...
            {post.image_url && (
                <div className="h-40 w-full bg-gray-800 rounded-xl mb-4 overflow-hidden border border-gray-700 relative">
                    ...
                    <div className="absolute bottom-2 right-2 bg-black/80 px-2 py-1 rounded text-xs text-white flex items-center gap-1"><ImageIcon size={12}/> 1</div>
                </div>
            )}
            
            <div className="text-[10px] text-gray-500 uppercase tracking-widest border-t border-gray-800 pt-3 flex justify-between items-center">
                <span className="bg-gray-800 px-2 py-1 rounded">{post.likes_count || 0} Likes</span>
